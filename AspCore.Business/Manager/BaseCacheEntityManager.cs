...
namespace AspCore.Business.Manager
{
      ...
      where TEntity : class, ICacheEntity, new()
    {
        ...
        public override ServiceResult<bool> AddList(List<TEntity> entityList)
        {
            _transactionBuilder.BeginTransaction();
            ...
            try
            {
                ServiceResult<bool> resultDAL = _dataAccess.Add(entityList.ToArray());
                if (resultDAL.IsSucceeded)
                {
                    ServiceResult<bool> resultCache = _cacheClient.Create(entityList.ToArray());
                    if (resultCache.IsSucceeded)
                    {
                        _transactionBuilder.CommitTransaction();
                        result.IsSucceeded = true;
        ...
        public override ServiceResult<bool> UpdateList(List<TEntity> entityList)
        {
            _transactionBuilder.BeginTransaction();
            ...
            try
            {
                ServiceResult<bool> resultDAL = _dataAccess.Update(entityList.ToArray());
                if (resultDAL.IsSucceeded)
                {
                    ServiceResult<bool> resultCache = _cacheClient.Update(entityList.ToArray());
                    if (resultCache.IsSucceeded)
                    {
                        _transactionBuilder.CommitTransaction();
                        result.IsSucceeded = true;
        ...
        public override ServiceResult<bool> DeleteList(List<TEntity> entityList)
        {
            _transactionBuilder.BeginTransaction();
            ...
            try
            {
                ServiceResult<bool> resultDAL = _dataAccess.Delete(entityList.ToArray());
                if (resultDAL.IsSucceeded)
                {
                    ServiceResult<bool> resultCache = _cacheClient.Delete(entityList.ToArray());
                    if (resultCache.IsSucceeded)
                    {
                        _transactionBuilder.CommitTransaction();
                        result.IsSucceeded = true;
