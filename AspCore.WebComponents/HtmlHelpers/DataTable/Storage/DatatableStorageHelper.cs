...
namespace AspCore.WebComponents.HtmlHelpers.DataTable.Storage
{
    public static class DatatableStorageHelper
    {
            ...
            where TEntity : class
        {
            ...
            DatatableObject<TEntity> cookieObject = storage.GetOrMakeDatatablesDictionary<TEntity>(controllerName);
            ...
            if (!cookieObject.dataTablesDictionary.ContainsKey(datatableId))
            {
                ...
                string tokenStorageKey = storage.GetObject<string>(ApiConstants.Api_Keys.CUSTOM_TOKEN_STORAGE_KEY);
                if (!string.IsNullOrEmpty(tokenStorageKey))
                {
                    string datatableStorageKey = HelperConstant.DataTable.DATA_TABLE_COOKIE_KEY + "_" + tokenStorageKey;
   ...
   where TEntity : class
        ...
        private static DatatableObject<TEntity> GetOrMakeDatatablesDictionary<TEntity>(
            this IStorage storage, string controllerName) where TEntity : class
        {
            string tokenStorageKey = storage.GetObject<string>(ApiConstants.Api_Keys.CUSTOM_TOKEN_STORAGE_KEY);
            string datatableStorageKey = HelperConstant.DataTable.DATA_TABLE_COOKIE_KEY + "_" + tokenStorageKey;
